...
learn_tau <- function(S, W, A, Y,
  ...
  if (controls_only) {
    ...
    pseudo_outcome <- ifelse(
      abs(S[A == 0]-Pi$pred[A == 0]) < 1e-5, 0,
      (Y[A == 0]-theta[A == 0])/(S[A == 0]-Pi$pred[A == 0]))
    ...
    # R-transformations
    pseudo_outcome <- ifelse(abs(S-Pi$pred) < 1e-5, 0, (Y-theta)/(S-Pi$pred))
  ...
  if (method == "glmnet") {
    ...
    print("bases coefs: " %+% round(coefs, 5))
    ...
    X_unpenalized <- model.matrix(Y ~ -1 + W1*W2, data = data.frame(W))
    X_unpenalized_A1 <- model.matrix(Y ~ -1 + W1*W2, data = data.frame(W))
    X_unpenalized_A0 <- model.matrix(Y ~ -1 + W1*W2, data = data.frame(W))
    ...
    print(fit$hal_basis_list)
    print(ncol(X_unpenalized))
