...
learn_tau_split <- function(S,
  ...
  if (controls_only) {
    ...
    # design matrix
    X <- data.frame(W_aug)
    X_full <- data.frame(A, W_aug*A, (1-A), W_aug*(1-A))

    # counterfactual design matrices
    X_A1_counter <- X_A0_counter <- data.frame(1, W_aug)
    X_A1_counter_full <- data.frame(1, W_aug, 0, W_aug*0)
    X_A0_counter_full <- data.frame(0, W_aug*0, 1, W_aug)
  }
  ...
  if (!is.null(bias_working_model_formula)) {
    ...
    if (controls_only) {
      ...
      # A1 fit
      fit_A1 <- cv.glmnet(
        x = as.matrix(X[delta == 1 & A == 1, , drop = FALSE]),
        y = pseudo_outcome[delta == 1 & A == 1], intercept = TRUE,
        family = "gaussian", weights = pseudo_weights[delta == 1 & A == 1],
        keep = TRUE, nfolds = v_folds, alpha = 1, relax = TRUE
      )

      # A0 fit
      fit_A0 <- cv.glmnet(
        x = as.matrix(X[delta == 1 & A == 0, , drop = FALSE]),
        y = pseudo_outcome[delta == 1 & A == 0], intercept = TRUE,
        family = "gaussian", weights = pseudo_weights[delta == 1 & A == 0],
        keep = TRUE, nfolds = v_folds, alpha = 1, relax = TRUE
      )
    }
    non_zero_A1 <- which(as.numeric(coef(fit_A1, s = "lambda.min", gamma = 0)) != 0)
    non_zero_A0 <- which(as.numeric(coef(fit_A0, s = "lambda.min", gamma = 0)) != 0)
    non_zero <- which(c(as.numeric(coef(fit_A1, s = "lambda.min", gamma = 0)),
                        as.numeric(coef(fit_A0, s = "lambda.min", gamma = 0))) != 0)
    coefs_A1 <- coef(fit_A1, s = "lambda.min", gamma = 0)[non_zero_A1]
    coefs_A0 <- coef(fit_A0, s = "lambda.min", gamma = 0)[non_zero_A0]
    coefs <- c(coefs_A1, coefs_A0)
    ...
    if (controls_only) {
      ...
      # selected counterfactual bases
      x_basis <- as.matrix(X_full[, non_zero, drop = FALSE])
      x_basis_A1 <- as.matrix(X_A1_counter_full[, non_zero, drop = FALSE])
      x_basis_A0 <- as.matrix(X_A0_counter_full[, non_zero, drop = FALSE])
