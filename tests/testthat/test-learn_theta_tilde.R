...
test_learn_theta_tilde <- function(test_args) {
  set.seed(123)
...
test_that("learn_theta_tilde basic functionalities", {
  ...
  for (i in 1:length(test_args)) {
    ...
    res <- test_learn_theta_tilde(cur_args)
...
test_that("learn_theta_tilde error behavior", {
  ...
  expect_error(test_learn_theta_tilde(test_args))
})
