...
run_sim <- function(B,
  ...
  rct_prop_seq <- seq(rct_prop_min, rct_prop_max, rct_prop_step)

  # results
  all_psi_est <- vector(mode = "list", length = length(rct_prop_seq))
  all_psi_coverage <- vector(mode = "list", length = length(rct_prop_seq))
  all_psi_ci_lower <- vector(mode = "list", length = length(rct_prop_seq))
  all_psi_ci_upper <- vector(mode = "list", length = length(rct_prop_seq))
  all_escvtmle_prop_selected <- vector(mode = "list", length = length(rct_prop_seq))

  for (i in 1:length(rct_prop_seq)) {
    rct_prop <- rct_prop_seq[i]
    print(rct_prop)

    # calculate an appropriate n, such that RCT data has sample size 200
    n <- 200 / rct_prop
